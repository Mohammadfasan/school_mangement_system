...
const Event = () => {
  // MODIFIED: Load achievements from localStorage using helper
  const [achievementsData, setAchievementsData] = useState(getInitialEventsData);
  ...
  const handleSubmit = (e) => {
    ...
    if (isEditing) {
      ...
      const updatedData = achievementsData.map(ach => ach.id === formState.id ? { ...ach, ...updatedAchievement } : ach );
      setAchievementsData(updatedData);
      localStorage.setItem('events', JSON.stringify(updatedData)); // <-- SAVE TO LOCALSTORAGE
      ...
      const newAchievement = {
        ...
        image: formState.imagePreview || 'https://via.placeholder.com/400x300.png?text=Event',
      };

      // MODIFIED: Create new array and save to localStorage
      const updatedData = [...achievementsData, newAchievement];
      setAchievementsData(updatedData);
      localStorage.setItem('events', JSON.stringify(updatedData)); // <-- SAVE TO LOCALSTORAGE
  ...
  const handleDelete = (id) => {
    if (window.confirm('Are you sure you want to delete this event?')) {
      ...
      // MODIFIED: Create new array and save to localStorage
      const updatedData = achievementsData.filter(ach => ach.id !== id);
      setAchievementsData(updatedData);
      localStorage.setItem('events', JSON.stringify(updatedData)); // <-- SAVE TO LOCALSTORAGE
  ...
  return (
    <div className="p-4 sm:p-6">
      ...
      <div className="mt-10">
        <h2 className="text-xl font-semibold text-gray-700 mb-4">Event List</h2>
