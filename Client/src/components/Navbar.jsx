...
import NotificationPopup from '../pages/NotificationPopup'; // Step 1: Import the new component
...
const Navbar = () => {
  ...
  useEffect(() => {
    const pathToPage = {
      ...
      // We no longer need a '/notification' route here
    };
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      ...
      // Step 4: Add logic to close notification popup on outside click
  ...
  const handleNavigation = (pageName, route = '') => {
    ...
    setIsMenuOpen(false);
    setIsPagesOpen(false);
    setIsNotificationOpen(false); // Close notification popup on any navigation
    ...
    if (route) {
      ...
      switch(pageName) {
        ...
        // The 'Notification' case is no longer needed
        default: navigate('/');
      }
    }
  };
  
  // Step 5: Toggle function for notification popup
    ...
    >
      <div className='flex items-center justify-between'>
        ...
        <nav className='hidden md:flex space-x-7'>
          {/* ... (Your navItems mapping code remains unchanged) ... */}
           {navItems.map((item) => (
